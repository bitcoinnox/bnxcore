...
#include "ui_zbitcoinnoxcontroldialog.h"
...
std::list<std::string> ZBitcoinNoxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBitcoinNoxControlDialog::listMints;

ZBitcoinNoxControlDialog::ZBitcoinNoxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBitcoinNoxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBitcoinNoxControlDialog::~ZBitcoinNoxControlDialog()
{
    delete ui;
}

void ZBitcoinNoxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBitcoinNoxControlDialog::updateList()
{
...
void ZBitcoinNoxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBitcoinNoxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBitcoinNox_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBitcoinNoxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBitcoinNoxControlDialog::GetSelectedMints()
{
...
void ZBitcoinNoxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
