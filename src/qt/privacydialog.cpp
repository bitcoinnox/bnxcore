...
#include "zbitcoinnoxcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBNX ought to be enough for anybody." - Bill Gates, 2017
    ui->zBNXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBNXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBNX->setEnabled(false);
        ui->pushButtonMintzBNX->setToolTip(tr("zBNX is currently disabled due to maintenance."));

        ui->pushButtonSpendzBNX->setEnabled(false);
        ui->pushButtonSpendzBNX->setToolTip(tr("zBNX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBNXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBNX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBNX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBNX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBNX in ") +
...
void PrivacyDialog::on_pushButtonSpendzBNX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBNX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBNX
        sendzBNX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBNX
    sendzBNX();
}

void PrivacyDialog::on_pushButtonZBitcoinNoxControl_clicked()
{
    ZBitcoinNoxControlDialog* zBitcoinNoxControl = new ZBitcoinNoxControlDialog(this);
    zBitcoinNoxControl->setModel(walletModel);
    zBitcoinNoxControl->exec();
}

void PrivacyDialog::setZBitcoinNoxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBitcoinNoxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBNX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BitcoinNox Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBNXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBNX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BNX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBNX</b>";
    ...
    // use mints from zBitcoinNox selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBitcoinNoxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBitcoinNoxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbitcoinnox selector in case it was used
    ZBitcoinNoxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBitcoinNox Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BitcoinNox, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBitcoinNox Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBNX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBNX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBNX "));
    ui->labelzBNXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBNXSyncStatus->setVisible(fShow);
}
